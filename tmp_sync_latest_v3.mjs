...
async function syncTracksFromSheet() {
  ...
  for (const row of rows) {
    ...
    const sheetGenreManual = row['ジャンル'] || row['genre'] || "";
    let extractedGenre = sheetGenreManual;

    if (!extractedGenre && sheetReview) {
      const match = sheetReview.match(/■ジャンル\s*[\r\n]*\s*([^\r\n]+)/);
      if (match) {
        extractedGenre = match[1].trim();
      }
    }
    ...
    const updateData = {
      ...
      genre: extractedGenre || undefined
    };
