...
export default function SelectionListContainer({ initialPlaylists, subPlaylists, previewQuery, enableArtistMain }: SelectionListContainerProps) {
  ...
  const userStats = useMemo(() => {
    ...
    const combinedData = [...initialPlaylists, ...subPlaylists];
    ...
    return stats;
  }, [initialPlaylists, subPlaylists]);

  // 各プレイリストのシンクロ率を計算し、ソートした結果をメモ化
  const sortedPlaylists = useMemo(() => {
    const playlistsWithSync = initialPlaylists.map(list => {
      const key = normalizeName(list.userName);
      ...
      const isMultiUser = [...initialPlaylists, ...subPlaylists].filter(p => normalizeName(p.userName) === key).length > 1;
  ...
  }, [initialPlaylists, interested, sortMode, userStats, subPlaylists]);
        ...
        <div className="flex-1 max-w-xl bg-surface/95 border border-surface-border rounded-[2rem] p-6 backdrop-blur-sm shadow-xl">
          <div className="flex items-center gap-5 text-left">
            ...
            <p className="text-foreground text-sm md:text-lg font-bold leading-tight">
              あなたが<span className="text-foreground font-black px-1 underline decoration-[var(--color-cyan-400)] decoration-2 underline-offset-4">「気になる」</span>した{enableArtistMain ? '参加者' : '楽曲'}との一致度を<br className="hidden md:block" />
