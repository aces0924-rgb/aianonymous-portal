    ...
    <main className="min-h-screen bg-background text-white selection:bg-[var(--color-cyan-500)] selection:text-white font-sans overflow-x-hidden relative">
      ...
      <nav className="fixed top-0 left-0 w-full z-50 bg-black/70 backdrop-blur-md border-b border-white/30">
        <div className="max-w-7xl mx-auto px-6 py-4">
          <Link href={`/${eventSlug}/selections${previewQuery}`} className="inline-flex items-center gap-2 text-white hover:text-white/80 transition-colors text-sm font-bold group">
          ...
          <div className="pt-3 flex flex-col items-center gap-2">
             ...
             {/* Share Section (Moved to Header) */}
              <div className="animate-in fade-in zoom-in duration-700">
                <ShareButton userName={playlist.userName} id={dbId} basePostUrl={shareBasePostUrl} shareHashtag={shareHashtag} siteTitle={siteTitle} />
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10 text-left">
            {sortedTracks.map((t: any, idx: number) => (
              <div key={t?.id} className="relative">
                ...
                <TrackCard track={t} preview={preview} enableArtistMain={enableArtistMain} eventSlug={eventSlug} />
              </div>
            ))}
          </div>
        </div>
