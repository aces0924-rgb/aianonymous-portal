...
import IllustrationTrackCard from '@/components/IllustrationTrackCard';
import Link from 'next/link';
import Image from 'next/image';
import ShareButton from '@/components/ShareButton';
import { getIllustrationPlaylistById, getAllIllustrationPlaylistsByUserName } from '@/app/recommend/actions';
    ...
    <main className="min-h-screen bg-background text-white selection:bg-[var(--color-cyan-500)] selection:text-white font-sans overflow-x-hidden relative">
      ...
      <div className="absolute top-4 left-4 md:top-8 md:left-8 z-30">
        <Link href={`/${eventSlug}/selections/illustrations${previewQuery}`} className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-surface/80 backdrop-blur-md border border-surface-border hover:border-purple-500 hover:text-purple-400 transition-all group">
          ...
          <span className="text-xs md:text-sm font-black tracking-widest uppercase">推しイラスト一覧へ</span>
        </Link>
      </div>
                   ...
                   return (
                     <Link
                       ...
                       className={`px-6 py-2 rounded-full text-xs md:text-sm font-black transition-all ${
                         isActive 
                           ? "bg-purple-500 text-black shadow-[0_0_15px_rgba(168,85,247,0.5)] scale-105" 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-10 text-left">
            {sortedTracks.map((t: any, idx: number) => (
              <div key={t.id} className="relative">
                ...
                <IllustrationTrackCard track={t} preview={preview} />
              </div>
            ))}
          </div>
        </div>
