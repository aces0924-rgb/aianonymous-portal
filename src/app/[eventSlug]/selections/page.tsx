...
export default async function SelectionsIndexPage({ params, searchParams }: { params: Promise<{ eventSlug: string }>, searchParams: Promise<{ preview?: string }> }) {
  ...
  const [mainPlaylists, subPlaylists] = await Promise.all([
    prisma.userPlaylist.findMany({ where: { eventId: event.id },
      orderBy: { createdAt: 'desc' },
    }),
    prisma.userPlaylistSub.findMany({ where: { eventId: event.id },
      orderBy: { createdAt: 'desc' },
    })
  ]);
